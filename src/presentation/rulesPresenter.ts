...
export function displayRuleAdded(rule: RuleConfig, configPath: string): void {
  ...
  console.log(`${colors.symbols.success} ${colors.success('Config saved to:')} ${colors.path(configPath)}`);
...
export function displayRuleRemoved(ruleId: string, configPath: string): void {
  ...
  console.log(`${colors.symbols.success} ${colors.success('Configuration saved to:')} ${colors.path(configPath)}`);
}

export function displayRulesList(rules: RuleConfig[]): void {
  if (rules.length === 0) {
    console.log(`\n${colors.warning('No rules defined yet.')}`);
    console.log(`${colors.dim('Use')} ${colors.code('archctl rules add')} ${colors.dim('to add a rule.')}`);
    return;
  }
...
export function displayUnknownSubcommand(subcommand: string | undefined): void {
  console.error(`${colors.symbols.error} ${colors.error('Unknown subcommand:')} ${colors.bold(subcommand || '(none)')}`);
  console.log(`\n${colors.bold('Available subcommands:')}`);
  console.log(`  ${colors.code('archctl rules list')}                    ${colors.dim('# List all rules')}`);
  console.log(`  ${colors.code('archctl rules add')}                     ${colors.dim('# Add a custom rule (interactive)')}`);
  console.log(`  ${colors.code('archctl rules remove --id <rule-id>')}   ${colors.dim('# Remove a rule')}`);
}
